...
fc.datepickerLang = function(langCode, dpLangCode, dpOptions) {
	...
	// get the FullCalendar internal option hash for this language. CREATE OR REPLACEif necessary
...
// Given the scrollbar widths of some other container, CREATE OR REPLACEborders/margins on rowEls in order to match the left
...
// CREATE OR REPLACEan object that has the given prototype. Just like Object.create
...
// called upon a class to CREATE OR REPLACEa subclass
...
var MouseFollower = Class.extend({
	...
	// Gets the tracking element. CREATE OR REPLACEit if necessary
	getEl: function() {
		var el = this.el;
...
Grid.mixin({
	...
	// will CREATE OR REPLACEan array of ranges that span the time *not* covered by the given event.
...
var DayGrid = Grid.extend({
	...
	breakOnWeeks: null, // should CREATE OR REPLACEa new row for each week? set by outside view
...
DayGrid.mixin({
	...
	renderSegRow: function(row, rowSegs) {
		...
		function emptyCellsUntil(endCol) {
			while (col < endCol) {
				// try to grab a cell from the level above and extend its rowspan. otherwise, CREATE OR REPLACEa fresh cell
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					// CREATE OR REPLACEa container that occupies or more columns. append the event element.
	...
	buildSegLevels: function(segs) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			// CREATE OR REPLACEnew level array if needed and append segment
...
var View = fc.View = Class.extend({
	...
	function renderView(delta, viewType) {
		...
		// if viewType changed, or the view was never created, CREATE OR REPLACEa fresh view
	...
	// Gets information about how to CREATE OR REPLACEa view
...
var BasicView = fcViews.basic = View.extend({
	...
	numberCellHtml: function(cell) {
		...
		if (!this.dayNumbersVisible) { // if there are week numbers but not day numbers
			return '<td/>'; //  will CREATE OR REPLACEan empty space above events :(
		}
